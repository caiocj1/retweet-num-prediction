...
class EmbedWord2VecMLPModel(LightningModule):

    def __init__(self):
        super(EmbedWord2VecMLPModel, self).__init__()
        self.read_config()
    ...
    def read_config(self):
        ...
        self.embed_dims = word2vec_params['embed_dims']
    ...
    def build_model(self):
        ...
        self.embed = nn.Sequential(
            nn.Linear(self.vector_size, self.vector_size // 2),
            nn.ReLU(),
            nn.Linear(self.vector_size // 2, self.vector_size // 2),
            nn.ReLU(),
            nn.Linear(self.vector_size // 2, self.embed_dims)
        )

        self.input = nn.Linear(7 + self.embed_dims, self.layer_width)
    ...
    def forward(self, batch):
        text_vecs = batch[0][:, -self.vector_size:].float()
        text_embedded = self.embed(text_vecs)

        other_features = batch[0][:, :-self.vector_size].float()
        new_input = torch.cat([other_features, text_embedded], axis=1)

        encoding = self.input(new_input)
